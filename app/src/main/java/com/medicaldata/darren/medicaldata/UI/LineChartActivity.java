...
public class LineChartActivity extends AppCompatActivity {
    ...
    public static class PlaceholderFragment extends Fragment {
        ...
        private void generateValues() {
            ...
            String[] date = intent.getStringArrayExtra("Data");//X轴的标注
            type = intent.getStringExtra("Type");//X轴的标注
            yname = intent.getStringExtra("YName");
            MaxValue = intent.getIntExtra("MaxValue",100);

            for (int i = 0; i < date.length; i++) {
                mAxisXValues.add(new AxisValue(i).setLabel(date[i]));
            }

            if(type.equals("Float")||type.equals("Float1")){
                float[] score= intent.getFloatArrayExtra("Score");//图表的数据点
                numberOfPoints = score.length;
                for (int i = 0; i < score.length; i++) {
                    mPointValues.add(new PointValue(i, score[i]));
                }
            }else{
                int[] score= intent.getIntArrayExtra("Score");//图表的数据点
                numberOfPoints = score.length;
                for (int i = 0; i < score.length; i++) {
                    mPointValues.add(new PointValue(i, score[i]));
                }
        ...
        private class ValueTouchListener implements LineChartOnValueSelectListener {
            ...
            public void onValueSelected(int lineIndex, int pointIndex, PointValue value) {
              //  Toast.makeText(getActivity(), "Selected: " + value, Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(getActivity(),new String(mAxisXValues.get(pointIndex).getLabelAsChars())+"："+mPointValues.get(pointIndex).getY(), Toast.LENGTH_SHORT).show();
            }
